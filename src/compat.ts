 ...
 */
export function setModelSpecsOverride(
  specs: Record<string, ModelSpec> | null,
): void {
...
function getModelSpec(model: string): ModelSpec {
  ...
  if (family === "gemini")
    return {
      ...
      thinkingBudget: FALLBACK_THINKING_BUDGET,
      isThinking: true,
    };
  return {
    maxOutputTokens: 65536,
...
function normalizeInstructionsContent(
...
function contentToResponseInputBlocks(
...
function messagesFromAntigravityRequest(
...
function convertResponsesToChatRequest(
...
function responseUsageFromCompletion(
...
function buildResponsesOutput(completion: CompatCompletion): {
